...
\begin{itemize}
  ...
  \item Version-controlled code that is \emph{live-linked} into this document via GitHub and Overleaf
\end{itemize}

All code shown in this chapter is included directly from the course GitHub repository. When the repository is updated, the documentation updates automatically.
...
\inputminted{cpp}{code/primes/src/primes_module.cpp}

\subsection{Design Notes}

This implementation exposes two functions:
\begin{itemize}
  \item \texttt{is\_prime(int n)} — tests a single value
  \item \texttt{primes\_up\_to(int n)} — generates all primes up to a limit
\end{itemize}

These functions are deliberately written without any Python knowledge. They operate entirely in standard C++ types:
\begin{itemize}
  \item \texttt{int}
...
\inputminted{cpp}{src/primes/bindings.cpp}
...
\inputminted{toml}{pyproject.toml}
...
\inputminted{python}{tests/test_primes.py}
...
Encouraging students to diagnose these issues early helps them develop practical software engineering skills alongside algorithmic understanding.

\section{Pedagogical Takeaways}
...
By the end of this chapter, students should understand not just \emph{how} to wrap C++ with Python, but \emph{when} and \emph{why} this design pattern is appropriate in scientific and imaging systems.

\section{Extensions}

This framework can be extended to:
\begin{itemize}
  \item Expose C++ classes instead of free functions
  \item Bind Eigen or OpenCV data structures
  \item Enable multi-threading with \texttt{py::gil\_scoped\_release}
\end{itemize}

These topics naturally lead into advanced performance engineering and hybrid system design.
